...
test("ensureWorktree recreates a clean mismatched worktree", () => {
  const script = [
    'import { execFileSync } from "node:child_process";',
    'import * as fs from "node:fs";',
    'import * as os from "node:os";',
    'import * as path from "node:path";',
    `import { createWorktree, ensureWorktree } from ${JSON.stringify(worktreeModuleUrl)};`,
    'const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "taskcore-worktree-reuse-"));',
    'const repoPath = path.join(tmpDir, "repo");',
    'fs.mkdirSync(repoPath, { recursive: true });',
    'execFileSync("git", ["init", "--initial-branch=main"], { cwd: repoPath, stdio: "ignore" });',
    'execFileSync("git", ["config", "user.name", "Taskcore Tests"], { cwd: repoPath, stdio: "ignore" });',
    ...
    'execFileSync("git", ["commit", "-m", "init"], { cwd: repoPath, stdio: "ignore" });',
    'const worktreePath = path.join(tmpDir, "code-T1");',
    'createWorktree(repoPath, worktreePath, "task/T999", "main");',
    'ensureWorktree(repoPath, worktreePath, "task/T1", "main");',
    'const branch = execFileSync("git", ["branch", "--show-current"], { cwd: worktreePath, encoding: "utf-8" }).trim();',
    'if (branch !== "task/T1") throw new Error(`unexpected branch ${branch}`);',
  ].join("\n");
...
test("ensureWorktree rejects dirty mismatched worktrees", () => {
  const script = [
    'import { execFileSync } from "node:child_process";',
    'import * as fs from "node:fs";',
    'import * as os from "node:os";',
    'import * as path from "node:path";',
    `import { createWorktree, ensureWorktree } from ${JSON.stringify(worktreeModuleUrl)};`,
    'const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "taskcore-worktree-dirty-"));',
    'const repoPath = path.join(tmpDir, "repo");',
    'fs.mkdirSync(repoPath, { recursive: true });',
    'execFileSync("git", ["init", "--initial-branch=main"], { cwd: repoPath, stdio: "ignore" });',
    'execFileSync("git", ["config", "user.name", "Taskcore Tests"], { cwd: repoPath, stdio: "ignore" });',
    ...
    'const worktreePath = path.join(tmpDir, "code-T1");',
    'createWorktree(repoPath, worktreePath, "task/T999", "main");',
    'fs.writeFileSync(path.join(worktreePath, "README.md"), "# dirty\\n", "utf-8");',
    'try {',
    '  ensureWorktree(repoPath, worktreePath, "task/T1", "main");',
    '  throw new Error("expected ensureWorktree to fail");',
    '} catch (err) {',
    '  if (!String(err).includes("local changes")) throw err;',
    '}',
  ].join("\n");
