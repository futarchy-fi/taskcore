...
function buildReviewPrompt(core: Core, task: Task, config: Config): string {
  ...
  sections.push("");
  sections.push("Use the task CLI. `TASK_ID` and `TASKCORE_AGENT_ID` are already set for this session.");
  sections.push("");
  sections.push("```bash");
  sections.push(`# Re-read the latest review context if needed:`);
  sections.push(`task review read`);
  sections.push("");
  sections.push(`# Record notes as you go (repeat as needed):`);
  sections.push(`task review note "Observation about the code or evidence"`);
  sections.push("");
  sections.push(`# Finalize the review:`);
  sections.push(`task review approve "Why this passes"`);
  sections.push(`task review request-changes "What needs to change"`);
  sections.push(`task review reject "Fundamental issue requiring re-analysis"`);
  sections.push("```");
  sections.push("");
...
function appendAnalysisInstructions(sections: string[], task: Task, config: Config): void {
  ...
  sections.push(`task decide execute`);
  sections.push("");
  ...
  sections.push(`task decompose start`);
  sections.push("");
  sections.push(`# Option 3: Block (cannot proceed, missing info or impossible)`);
  sections.push(`task block "Why this cannot proceed"`);
  sections.push("```");
  sections.push("");
  sections.push("## Rules");
  sections.push("");
...
function appendDecompositionInstructions(sections: string[], task: Task, config: Config): void {
  ...
  sections.push("Use the task CLI. `TASK_ID` and `TASKCORE_AGENT_ID` are already set for this session.");
  sections.push("");
  sections.push("Use the incremental decompose flow — `task decompose start` also resumes an in-progress session:");
  sections.push("");
  sections.push("```bash");
  ...
  sections.push(`task decompose add "Subtask title" \\`);
  sections.push(`  --desc "What this child should do" \\`);
  sections.push(`  --cost 10`);
  sections.push("");
  sections.push(`# Step 3: Commit when all children are added`);
  sections.push(`task decompose commit "Brief description of your decomposition strategy"`);
  sections.push("```");
  sections.push("");
...
function appendExecutionInstructions(sections: string[], task: Task, config: Config): void {
  ...
  sections.push("");
  sections.push("```bash");
  sections.push(`# Optional progress note:`);
  sections.push(`task update "Short progress update"`);
  sections.push("");
  if (task.reviewConfig === null) {
    sections.push(`# Work complete → complete directly (no reviewer configured):`);
    sections.push(`task complete "Description of what you did"`);
  } else {
    sections.push(`# Work complete → submit for review:`);
    sections.push(`task submit "Description of what you did"`);
  }
  sections.push("");
  sections.push(`# Blocked → cannot proceed:`);
  sections.push(`task block "What is blocking you"`);
  sections.push("```");
  sections.push("");
  sections.push("## Rules");
  sections.push("");
